...
then
  ...
  for i in `ls $address`
  do
    cat $address/$i >> outfile_temp.cabbage
  done
  #Insert END-OF-FILE string into <cabbage datafile>:
  ./Step0-END-FILE >> outfile_temp.cabbage
fi
...
then
  ...
  for i in `ls $address`
  do
    ./Step0-cabbage-core $address/$i >> outfile_temp.cabbage
  done
  #Insert END-OF-FILE string into <cabbage datafile>:
  ./Step0-END-FILE >> outfile_temp.cabbage
fi
...
then
  if [[ "$2" == "-type2" || "$2" == "-type3" ]]
  then
    ../Step3-PM_hybrid outfile_temp.cabbage
  else
